...
class GraphConstruction ():
    def __init__(self, raw_data_dir, graph_data_dir):

        # Read edge index
        edge_index_drug_adr = self._read_edge_index(raw_data_dir, graph_data_dir['edge_index']['drug_adr'])
        edge_index_gene_drug = self._read_edge_index(raw_data_dir, graph_data_dir['edge_index']['gene_drug'])
        edge_index_disease_dp = self._read_edge_index(raw_data_dir, graph_data_dir['edge_index']['disease_dp'])                                        
        edge_index_gene_disease = self._read_edge_index(raw_data_dir, graph_data_dir['edge_index']['gene_disease'])                                          
        edge_index_gene_gene = self._read_edge_index(raw_data_dir, graph_data_dir['edge_index']['gene_gene'])
        
        # Read node attributes

        if os.path.exists(os.path.join(raw_data_dir, graph_data_dir['node_attr']['drug']+'.csv')):
            node_attr_drug = self._read_node_attr(raw_data_dir, graph_data_dir['node_attr']['drug'])
        else:
        ...
        if os.path.exists(os.path.join(raw_data_dir, graph_data_dir['node_attr']['adr']+'.csv')):
            node_attr_adr = self._read_node_attr(raw_data_dir, graph_data_dir['node_attr']['adr'])
        else:
        ...
        if os.path.exists(os.path.join(raw_data_dir, graph_data_dir['node_attr']['disease']+'.csv')):
            node_attr_disease = self._read_node_attr(raw_data_dir, graph_data_dir['node_attr']['disease'])
        else:
        ...
        if os.path.exists(os.path.join(raw_data_dir, graph_data_dir['node_attr']['dp']+'.csv')):
            node_attr_dp = self._read_node_attr(raw_data_dir, graph_data_dir['node_attr']['dp'])
        else:
        ...
        if os.path.exists(os.path.join(raw_data_dir, graph_data_dir['node_attr']['gene']+'.csv')):
            node_attr_gene = self._read_node_attr(raw_data_dir, graph_data_dir['node_attr']['gene'])
        else:
    ...
    def _read_edge_index(self, raw_data_dir, file_name):
        edge_index = pd.read_csv(os.path.join(raw_data_dir, file_name+'.csv'))
    ...
    def _read_node_attr (self, raw_data_dir, file_name):
        node_attr = pd.read_csv(os.path.join(raw_data_dir, file_name+'.csv'))
