...
disease_gene = readRDS("disease_gene.rds")

drugbank_name = unique(drug_se[,c(1,4)])
meddra_name = unique(drug_se[,c(2,3)])

disease_name = unique(disease_symptom[,c(1,5)])
hpo_name = unique(disease_symptom[,c(2,4,3)])

######
# shuffle drug targets
drug_target[,2] = sample(PPI_genes, nrow(drug_target))
disease_gene[,2] = sample(PPI_genes, nrow(disease_gene))

# shuffle drugs
S1 = sample(c(unique(drug_se[, 1]), sample(unique(drug_se[, 1]), nrow(drug_se) - length(unique(drug_se[, 1])), replace = TRUE)))
drug_se = drug_se[,-4]
drug_se[,1] = S1
drug_se = merge(drug_se, drugbank_name, by = "drugbank_id") 
drug_se = drug_se[, c(1,2,4,3)]


# shuffle ADRs
S3 = sample(c(unique(drug_se[, 2]), sample(unique(drug_se[, 2]), nrow(drug_se) - length(unique(drug_se[, 2])), replace = TRUE)))
drug_se = drug_se[,-4]
drug_se[,2] = S3
drug_se = merge(drug_se, meddra_name, by = "meddra_id") 
drug_se = drug_se[, c(2,1,3,4)]


# shuffle diseases
S2 = sample(c(unique(disease_symptom[, 1]), sample(unique(disease_symptom[, 1]), nrow(disease_symptom) - length(unique(disease_symptom[, 1])), replace = TRUE)))
disease_symptom = disease_symptom[,-5]
disease_symptom[,1] = S2
disease_symptom = merge(disease_symptom, disease_name, by = "mondo_id") 
disease_symptom = disease_symptom[, c(1,2,5,4,3)]

# shuffle symptome
S4 = sample(c(unique(disease_symptom[, 2]), sample(unique(disease_symptom[, 2]), nrow(disease_symptom) - length(unique(disease_symptom[, 2])), replace = TRUE)))
disease_symptom = disease_symptom[,1:3]
disease_symptom[,2] = S4
disease_symptom = merge(disease_symptom, hpo_name, by = "hpo_id") 
disease_symptom = disease_symptom[, c(2,1,3,4,5)]
#################################

ADRs = unique(drug_se$meddra_id)
DPs = unique(disease_symptom$hpo_id)
Drugs = unique(drug_se$drugbank_id)
Diseases = unique(disease_gene$mondo_id)
...
for(i in Diseases){
  ...
  gene_Di = unique(disease_gene[disease_gene$mondo_id == i,2])
...
####add names to nodes
ADR_names = unique(drug_se[drug_se$meddra_id %in% ADR_Node[,1],c(2,4)])
...
Drug_names = unique(drug_se[drug_se$drugbank_id %in% Drug_Node[,1],c(1,3)])
...
Disease_names = unique(disease_symptom[disease_symptom$mondo_id %in% Disease_Node[,1],c(1,3)])
...
#####Save data
write.table(edge_index_homo, "edge_index_random.csv", sep = ",")
write.table(Conversion_table, "conversion_table_random.csv", sep = ",")
