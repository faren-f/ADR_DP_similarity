...
setwd("../data/")
drug_se = readRDS("drug_adr_cardiacIndicatedDrugs_Removed.rds")
...
all_adrs = unique(drug_se$meddra_id)
all_drugs = unique(drug_se$drugbank_id)

disease_symptom = disease_symptom[disease_symptom$meddra_id %in% all_adrs, ]
diseases = unique(disease_symptom$mondo_id)

drug_target = drug_target[drug_target$drugbank_id %in% all_drugs, ]
disease_gene_protein = disease_gene_protein[disease_gene_protein$mondo_id %in% diseases,]
...
####add names to nodes
ADR_names = unique(drug_se[drug_se$meddra_id %in% ADR_Node[,1],2:3])
...
#####Save data
write.table(edge_index_homo, "edge_index.csv", sep = ",")
write.table(Conversion_table, "ConversionTable.csv", sep = ",")
